...
import { updateJob, pushLog } from "./jobs";
import {
  extractHtmlAssets,
  extractCssAssets,
  classifyAsset,
...
const EXPORTS_DIR = path.join(process.cwd(), "db", "exports");

const MAX_FILE_BYTES = 15 * 1024 * 1024; // 15 MB per asset
const MAX_TOTAL_BYTES = 250 * 1024 * 1024; // 250 MB per export
...
export async function runExportJob(jobId: string, params: ExportParams): Promise<void> {
  ...
  try {
    ...
    updateJob(jobId, { phase: "downloading", phaseLabel: `Téléchargement des pages (0/${pages.length})…` });
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      try {
        ...
        pushLog(jobId, `Page récupérée : ${pagePathOf(pageUrl)}`);
      } catch {
        pushLog(jobId, `⚠ Échec de récupération : ${pagePathOf(pageUrl)} — ignorée`);
      } finally {
        clearTimeout(t);
      }
      updateJob(jobId, {
        pagesDone: i + 1,
        phaseLabel: `Téléchargement des pages (${i + 1}/${pages.length})…`,
      });
    }
    ...
    updateJob(jobId, { phase: "assets", phaseLabel: "Collecte des actifs (images, polices, CSS, JS)…" });

    // Asset URLs referenced by the pages
    const pending = new Set<string>();
    for (const [, html] of pageHtmls) {
      for (const u of extractHtmlAssets(html, siteUrl)) {
        // Respect the user's asset choices
    ...
    async function worker() {
      while (qHead < queue.length) {
        ...
        if (processed % 8 === 0 || processed === queue.length) {
          updateJob(jobId, {
            filesCount: files.size,
            phaseLabel: `Téléchargement des actifs… ${files.size} fichiers (${processed}/${queue.length})`,
          });
        }
        if (totalBytes > MAX_TOTAL_BYTES) {
          pushLog(jobId, "⚠ Limite de taille atteinte — certains actifs distants resteront liés au site d'origine.");
          break;
        }
      }
    }
    ...
    pushLog(jobId, `${files.size} actifs téléchargés (${(totalBytes / 1024).toFixed(0)} Ko)`);
    ...
    updateJob(jobId, { phase: "packaging", phaseLabel: "Réécriture du HTML et préparation du ZIP…" });
    ...
    for (const [pageUrl, html] of pageHtmls) {
      ...
      pushLog(jobId, `Fichier généré : ${localFile}`);
    }
    ...
    // ── Phase 4: build ZIP ──
    updateJob(jobId, { phase: "packaging", phaseLabel: `Création de l'archive ZIP… (${files.size} fichiers)` });
    const zip = new JSZip();
    ...
    const zipName = `${safeFolder(new URL(siteUrl).hostname)}-${new Date().toISOString().slice(0, 10)}-${jobId.slice(0, 6)}.zip`;
    ...
    updateJob(jobId, {
      status: "done",
      phase: "done",
      phaseLabel: "Exportation terminée",
      filesCount: files.size,
      bytes: totalBytes + zipBuf.length,
      zipPath,
      zipName,
      finishedAt: Date.now(),
    });
    pushLog(jobId, `Archive prête : ${zipName} (${(zipBuf.length / 1024).toFixed(0)} Ko)`);
  } catch (err) {
    sawFatal = err instanceof Error ? err.message : String(err);
    updateJob(jobId, {
      status: "error",
      phase: "error",
      error: sawFatal,
      phaseLabel: "Échec de l'exportation",
      finishedAt: Date.now(),
    });
    pushLog(jobId, `✖ Erreur : ${sawFatal}`);
  }
}
